...
public class RungeKutta4 implements ODESolver {
    ...
    public static void main(String[] args) {
        ...
        RungeKutta4 solver = new RungeKutta4(0, 1, 0.05, initialValues, variables, equations, "t");
        solver.calculate();
        double[][] rkSolution = solver.getMatrix();
        List<List<Double>> rkSolutionList = new ArrayList<>();
        for (int i = 0; i < rkSolution.length; i++) {
            List<Double> variableValues = new ArrayList<>();
            for (int j = 0; j < rkSolution[i].length; j++) {
                variableValues.add(rkSolution[i][j]);
            }
            rkSolutionList.add(variableValues);
        }

        // Print the extracted solution for verification
        for (int i = 0; i < rkSolutionList.size(); i++) {
            //System.out.println("Variable " + variables[i] + " values: " + rkSolutionList.get(i));
            System.out.println(rkSolutionList.get(i));
