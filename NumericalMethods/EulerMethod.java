...
public class EulerMethod implements ODESolver {
    ...
    public static void main(String [] args){
        ...
        double[] start = {0,1,0.05}; // intial values of indep, final value and step size
        EulerMethod euler = new EulerMethod(start, initialValues, equations, indep, vars);
        euler.calculate();
        double[][] eulerSolution = euler.getMatrix();
        List<List<Double>> eulerSolutionList = new ArrayList<>();
        for (int i = 0; i < eulerSolution.length; i++) {
            List<Double> variableValues = new ArrayList<>();
            for (int j = 0; j < eulerSolution[i].length; j++) {
                variableValues.add(eulerSolution[i][j]);
            }
            eulerSolutionList.add(variableValues);
        }

        // Print the extracted solution for verification
        for (int i = 0; i < eulerSolutionList.size(); i++) {
            //System.out.println("Variable " + vars[i] + " values: " + eulerSolutionList.get(i));

            System.out.println(eulerSolutionList.get(i));
        }
